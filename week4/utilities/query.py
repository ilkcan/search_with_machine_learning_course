...
def create_query(user_query, click_prior_query, sort="_score", sortDir="desc", size=10, source=None, categories = [], should_boost_categories = False):
...
def search(client, user_query, index="bbuy_products_with_embeddings", sort="_score", sortDir="desc", should_boost_categories=False, is_vector=False):
    ...
    if is_vector:
        ...
        query_obj = create_query(user_query, click_prior_query=None, sort=sort, sortDir=sortDir, source=["name", "shortDescription"], categories=categories, should_boost_categories=should_boost_categories)
    print(query_obj)
...
if __name__ == "__main__":
    ...
    general.add_argument("-i", '--index', default="bbuy_products_with_embeddings",
    ...
    for line in sys.stdin:
        ...
        if query == "Exit":
            break
        search(client=opensearch, user_query=query, index=index_name, should_boost_categories=args.should_boost_categories, is_vector=is_vector)
