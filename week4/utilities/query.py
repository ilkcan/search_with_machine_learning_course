...
def create_query(user_query, click_prior_query, sort="_score", sortDir="desc", size=10, source=None, categories = [], should_boost_categories = False):
    ...
    elif not should_boost_categories:
        cat_filter = {
            "terms": {
...
def search(client, user_query, index="bbuy_products", sort="_score", sortDir="desc", should_boost_categories=False, is_vector=False):
...
if __name__ == "__main__":
    ...
    general.add_argument("-v", '--vector', action=argparse.BooleanOptionalAction, help='Determines whether vector search will be used.')
