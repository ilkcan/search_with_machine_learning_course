...
query_classifier = fasttext.load_model("../week3/output/query_classifier_150k.bin")
...
# Creates vector query for embedding field
def create_vector_query(user_query, size=10, source=None):
    arr = [user_query]
    query_embedding = sentence_transformer.encode(arr)
...
def search(client, user_query, index="bbuy_products", sort="_score", sortDir="desc", is_boost=False, is_vector=False, category_threshold=0.3):
    ...
    categories, probs = query_classifier.predict(normalized_query, k=candidate_count)
    ...
    if is_vector:
        ...
        query_obj = create_query(user_query, click_prior_query=None, filters=filters, sort=sort, sortDir=sortDir,\
            source=["name", "shortDescription"], categories=category_list, is_boost=is_boost)
    # logging.info(query_obj)
    response = client.search(query_obj, index=index, request_timeout=600, timeout="600s")
...
if __name__ == "__main__":
    ...
    general.add_argument("-v", '--vector', default=False, action='store_true', help='Determines whether vector search will be used.')
    general.add_argument("-b", '--boost', default=False, action='store_true', help='Determines whether predicted categories should be used as boost.')
    general.add_argument("-c", '--catthreshold', type=float, default=0.3, help='Probability threshold for categories')
    ...
    for line in sys.stdin:
        ...
        if query == "Exit":
            break
        search(client=opensearch, user_query=query, index=index_name, is_boost=is_boost, is_vector=is_vector, category_threshold=category_threshold)
